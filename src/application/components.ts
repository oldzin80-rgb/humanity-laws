...
function renderCompanionPanel(page: PageModel): string {
  ...
  return `<section class="conversation-panel" aria-label="${companion} conversation"><div><p class="eyebrow">Conversation</p><h2>Talk with ${companion}</h2><p>Write one honest message. ${companion} will answer as an AI companion, with source transparency and human judgment preserved.</p></div><form data-companion-form="${companion}" class="conversation-form"><label for="companion-input-${companion.toLowerCase()}">Your message</label><textarea id="companion-input-${companion.toLowerCase()}" name="input" maxlength="2000" rows="5" placeholder="${escapeHtml(placeholder)}" required></textarea><button class="button button-primary" type="submit">Send to ${companion}</button><p class="auth-message" role="status" data-companion-status></p></form><article class="conversation-output" data-companion-output aria-live="polite"><p class="eyebrow">Response</p><p>Your response will appear here after you send a message.</p></article></section>`;
}

function companionClientScript(): string {
  return `(()=>{const form=document.querySelector("[data-companion-form]");if(!form)return;const companion=form.getAttribute("data-companion-form");const status=form.querySelector("[data-companion-status]");const output=document.querySelector("[data-companion-output]");const getSession=()=>{try{const s=JSON.parse(localStorage.getItem("hl_auth_session_v1")||"null");return s&&s.expiresAt>Date.now()?s:null}catch{return null}};const say=t=>{if(status)status.textContent=t};const render=d=>{if(!output)return;const esc=v=>String(v||"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;");output.innerHTML='<p class="eyebrow">Response</p><p>'+esc(d.message)+'</p><small>'+esc(d.humanSovereigntyReminder||"Your human judgment remains final.")+'</small>'+(d.sourceSummary?'<small>'+esc(d.sourceSummary)+'</small>':"")};form.addEventListener("submit",async e=>{e.preventDefault();const session=getSession();if(!session?.accessToken){say("Sign in to talk with "+companion+".");return}const input=String(new FormData(form).get("input")||"").trim();if(input.length<2){say("Write a short message first.");return}try{say("Sending...");const r=await fetch("/api/companion",{method:"POST",headers:{"Authorization":"Bearer "+session.accessToken,"Content-Type":"application/json"},body:JSON.stringify({companion,input})});const d=await r.json();if(!r.ok||!d.success)throw new Error(d.error||"Conversation failed.");render(d);say("Response ready.")}catch(error){say(error instanceof Error?error.message:"Conversation failed.")}})})();`;
}
