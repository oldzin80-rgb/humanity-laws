...
function renderSparkRitualPanel(page: PageModel): string {
  ...
  const modes = ["Morning", "Evening", "Discovery", "Circle", "Table", "Council", "Book"];
  const supports = ["Adam", "Eve", "Council", "Book", "Table", "Wellness", "Library", "Podcast"];
  return `<section class="spark-ritual" aria-label="Spark ritual system" data-spark-ritual><div><p class="eyebrow">Spark ritual</p><h2>Choose one main mode.</h2><p>Use one primary mode and up to two supports. If too many are selected, Spark keeps the experience focused.</p><div class="spark-chip-row" aria-label="Primary Spark modes">${modes.map((mode, index) => `<span class="spark-chip${index === 0 ? " spark-chip-active" : ""}">${escapeHtml(mode)}</span>`).join("")}</div><p class="spark-help">Founder Podcast Mode is creator-only. Publishing is not live until provider verification.</p></div><div class="spark-orb-grid" aria-label="Wheel and dice state"><article><span>Wheel</span><strong>Category reveal</strong><small>Reduced-motion fallback ready.</small></article><article><span>Dice</span><strong>Depth variation</strong><small>No casino feel. No pressure loop.</small></article><article><span>Save</span><strong>Library-ready</strong><small>Local first; server-ready later.</small></article></div><div class="spark-supports" aria-label="Supporting Spark modes"><p class="eyebrow">Supports</p>${supports.map((mode) => `<span>${escapeHtml(mode)}</span>`).join("")}</div><p class="spark-help">Morning Spark reminders coming soon. Notifications will be connected after provider verification.</p></section>`;
}
...
function companionExtraCss(): string {
  return `.check-row{display:flex!important;grid-template-columns:auto 1fr;align-items:flex-start;gap:10px;font-weight:600!important;line-height:1.45}.check-row input{width:auto!important;margin-top:4px}.conversation-history{margin-top:18px;padding:18px;border:1px solid rgba(255,255,255,.1);border-radius:24px;background:rgba(0,0,0,.16)}.history-turn{border-top:1px solid rgba(255,255,255,.09);padding-top:12px;margin-top:12px}.history-turn strong{color:var(--gold)}.history-turn p{font-size:.95rem!important;margin:4px 0 10px!important;line-height:1.55!important}.response-next-steps{margin:14px 0;padding:14px;border:1px solid rgba(216,184,102,.24);border-radius:18px;background:rgba(216,184,102,.06)}.response-next-steps strong{color:var(--gold)}.response-next-steps ul{margin:8px 0 0;padding-left:20px;color:var(--soft);line-height:1.55}.avatar-presence,.spark-ritual{display:grid;grid-template-columns:minmax(0,1fr) minmax(220px,340px);gap:18px;align-items:center;margin:18px 0;padding:clamp(22px,4vw,38px);border:1px solid var(--line);border-radius:34px;background:linear-gradient(145deg,rgba(216,184,102,.08),rgba(255,255,255,.028));box-shadow:0 22px 70px rgba(0,0,0,.22)}.avatar-presence h2,.spark-ritual h2{font-size:clamp(1.6rem,3vw,2.7rem)}.avatar-presence small,.spark-ritual small,.spark-help{display:block;color:var(--gray);line-height:1.5}.avatar-stage,.spark-chip-row,.spark-supports{display:flex;gap:12px;justify-content:flex-start;flex-wrap:wrap}.avatar-stage{justify-content:flex-end}.avatar-stage span{display:grid;place-items:center;min-width:116px;min-height:116px;border:1px solid rgba(216,184,102,.34);border-radius:999px;background:radial-gradient(circle at 35% 28%,rgba(255,255,255,.14),rgba(216,184,102,.08) 44%,rgba(0,0,0,.22));color:var(--soft);font-size:.84rem;text-align:center;padding:18px}.spark-chip,.spark-supports span{border:1px solid rgba(216,184,102,.26);border-radius:999px;padding:10px 13px;color:var(--soft);background:rgba(255,255,255,.04);font-size:.9rem}.spark-chip-active{background:linear-gradient(135deg,#f5df99,#b98d3d);color:#080604;font-weight:800}.spark-orb-grid{display:grid;gap:10px}.spark-orb-grid article{border:1px solid rgba(216,184,102,.24);border-radius:22px;padding:16px;background:rgba(0,0,0,.16)}.spark-orb-grid span{display:block;color:var(--gold);font-size:.78rem;text-transform:uppercase;letter-spacing:.14em}.spark-orb-grid strong{display:block;margin:5px 0;color:var(--ivory)}.spark-supports{grid-column:1/-1}.spark-supports .eyebrow{width:100%}@media(max-width:980px){.avatar-presence,.spark-ritual{grid-template-columns:1fr}.avatar-stage{justify-content:flex-start}}@media(prefers-reduced-motion:reduce){*{scroll-behavior:auto!important;transition:none!important}.spark-ritual{background:rgba(255,255,255,.045)}}`;
}
